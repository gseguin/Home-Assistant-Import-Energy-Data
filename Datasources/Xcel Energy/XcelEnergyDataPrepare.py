...
def customReadInputFile(inputFileName: str) -> pd.DataFrame:
    ...
    if has_gas:
        gas_rows = len(dataFrame[dataFrame["Gas Usage (Therms)"].notna() & (dataFrame["Gas Usage (Therms)"] != 0)])
    ...
    if has_electric:
        electric_rows = len(dataFrame[dataFrame["Electric Usage (kWh)"].notna() & (dataFrame["Electric Usage (kWh)"] != 0)])
